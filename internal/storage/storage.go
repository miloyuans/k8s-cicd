...
func EnsureDailyFile(fileName string, client dynamic.Interface, cfg *config.Config) error {
	if _, err := os.Stat(fileName); os.IsNotExist(err) {
		if prefix := strings.TrimSuffix(filepath.Base(fileName), filepath.Ext(fileName)); strings.HasPrefix(prefix, "deploy_") && client != nil {
...
func UpdateAllDeploymentVersions(cfg *config.Config, client dynamic.Interface) {
	...
	updated := false
	seen := make(map[string]map[string]bool) // env -> service
