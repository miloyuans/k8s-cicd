...
func StartBot(cfg *config.Config, q *queue.Queue) {
    ...
    for category := range cfg.TelegramBots {
        if _, ok := cfg.TelegramChats[category]; !ok {
            log.Printf("Warning: No chat ID configured for category %s", category)
        }
    }
    if _, ok := cfg.TelegramChats["other"]; !ok {
        log.Printf("Warning: No default chat ID configured for category 'other'")
    }
    if _, ok := cfg.TelegramBots["other"]; !ok {
        log.Printf("Warning: No bot configured for category 'other'")
    }
    if cfg.DeployCategory != "" && cfg.DeployCategory != "other" {
        if _, ok := cfg.TelegramChats[cfg.DeployCategory]; !ok {
            log.Printf("Warning: No chat ID configured for deploy category %s", cfg.DeployCategory)
        }
        if _, ok := cfg.TelegramBots[cfg.DeployCategory]; !ok {
            log.Printf("Warning: No bot configured for deploy category %s", cfg.DeployCategory)
        }
    }
...
func logToFile(message string, result *storage.DeployResult) error {
    ...
    logEntry := fmt.Sprintf("[%s] %s: service=%s, env=%s, version=%s, success=%t, error=%s\n",
        time.Now().Format("2006-01-02 15:04:05"), message, result.Request.Service, result.Request.Env, result.Request.Version, result.Success, result.ErrorMsg)
