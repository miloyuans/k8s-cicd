...
func (a *Approval) periodicQueryAndSync() {
	...
	for range ticker.C {
			...
			for _, env := range envs {
				...
				for i := range tasks {
					...
					if len(task.Environments) == 1 {
						// 单环境: 正常存储
						if err := a.mongo.StoreTaskIfNotExistsEnv(*task, task.Environments[0]); err != nil {
							logrus.WithFields(logrus.Fields{
								"time":   time.Now().Format("2006-01-02 15:04:05"),
								"method": "periodicQueryAndSync",
								"task_id": task.TaskID,
							...
							storedCount++
							a.handleStoredTask(task.TaskID, task.Environments[0]) // 统一验证
						}
					} else {
						// 多环境: 拆分复制，每个 env 独立 TaskID 和存储
						logrus.WithFields(logrus.Fields{
							"time":   time.Now().Format("2006-01-02 15:04:05"),
							"method": "periodicQueryAndSync",
							"task_id": task.TaskID,
							"environments": task.Environments,
						}).Infof("检测到多环境任务，开始拆分存储")

						for _, subEnv := range task.Environments {
							if !contains(a.cfg.Query.ConfirmEnvs, subEnv) {
								logrus.Debugf("跳过多环境子任务非确认环境: %s", subEnv)
								continue
							}
							...
							if err := a.mongo.StoreTaskIfNotExistsEnv(taskCopy, subEnv); err != nil {
								...
								storedCount++
								a.handleStoredTask(taskCopy.TaskID, subEnv) // 每个子任务独立验证
							}
						}
						...
						logrus.WithFields(logrus.Fields{
							...
							"environments": task.Environments,
						}).Infof("多环境任务拆分完成，存储 %d 个子任务", storedCount)
					}
		...
		}).Infof("查询同步完成，本轮新增 %d 个任务 (含多环境拆分)", totalNewTasks)
	}
}

// 新增: handleStoredTask 统一处理存储后验证 (提取复用)
func (a *Approval) handleStoredTask(taskID, env string) {
	logrus.WithFields(logrus.Fields{
		"time":   time.Now().Format("2006-01-02 15:04:05"),
	...
	// 立即更新状态为 待确认（冗余确保）
	updateErr := a.mongo.UpdateTaskStatus(taskID, "待确认", "system")
	if updateErr != nil {
		logrus.WithFields(logrus.Fields{
			"time":   time.Now().Format("2006-01-02 15:04:05"),
			"method": "handleStoredTask",
	...
	if latestTask.ConfirmationStatus != "待确认" {
		logrus.WithFields(logrus.Fields{
			"time":   time.Now().Format("2006-01-02 15:04:05"),
			"method": "handleStoredTask",
			"task_id": taskID,
		...
		}).Errorf("最新状态非 待确认，强制更新")
		a.mongo.UpdateTaskStatus(taskID, "待确认", "system")
	...
	}).Infof("=== 子任务存储成功，状态变更完成 (待确认): status=%s, popup_sent=%t ===\nFull Latest Data: %+v",
