...
func StartWorkers(cfg *config.Config, mongo *client.MongoClient, k8s *kubernetes.K8sClient, botMgr *telegram.BotManager, apiClient *api.APIClient) {
	startTime := time.Now()
	// 步骤1：添加等待组
	q.wg.Add(q.workers)
	// 步骤2：启动worker
...
func (q *TaskQueue) executeTask(cfg *config.Config, mongo *client.MongoClient, k8s *kubernetes.K8sClient, apiClient *api.APIClient, task *models.Task, botMgr *telegram.BotManager) error {
	...
	if err != nil {
		...
		}).Errorf(color.RedString("等待就绪失败: %v", err))
	...
	// 步骤5：发送通知
	botMgr.SendNotification(task.Service, namespace, task.User, oldVersion, task.Version, success)
	...
	mongo.UpdateTaskStatus(task.Service, task.Version, namespace, task.User, status)
